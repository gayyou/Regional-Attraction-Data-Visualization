...
//     pLine.setMap(this.map); 
  ...
  methods: {
    ...
    getPolygon(item, rankUnit) {
      ...
      // let flightPath = new window.AMap.Polygon({
      //   path: arr,
      //   strokeColor: "#000",
      //   strokeOpacity: 0.3,
      //   borderWeight: 0.5,
      ...
      poly.value = item.value;
      poly.id = item.id;
      
      window.google.maps.event.addDomListener(poly, 'click', (event) => {
        ...
        this.coordInfoWindow.setContent(['id:' + poly.id,
                                    'value:' + poly.value].join('<br>'));
